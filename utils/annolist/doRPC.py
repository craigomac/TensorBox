...
import os, sys
import utils.annolist.AnnotationLib as annlib
...
def asort(idlGT, idlDet, minWidth, minHeight, style, minCover, minOverlap, maxDistance, maxWidth=float('inf'),
    ...
    for x, anno in enumerate(idlGT):
        ...
        for i, filterAnno in enumerate(idlDet):
            if (annlib.suffixMatch(anno.imageName, filterAnno.imageName) and anno.frameNr == filterAnno.frameNr):
                filterIndex = i
                imageFound = True
                break
...
def comp_prec_recall_all_params(annoIDL, detIDL, ignoreIDL, minWidth=0, minHeight=0, maxWidth=float('inf'),
    ...
    for anno in annoIDL:
        for j, detAnno in enumerate(detIDL):
            if (annlib.suffixMatch(anno.imageName, detIDL[j].imageName) and anno.frameNr == detIDL[j].frameNr):
    ...
    for i in range(len(annoIDL)):
        ...
        for j, detAnno in enumerate(detIDL):
            if (annlib.suffixMatch(annoIDL[i].imageName, detIDL[j].imageName) and annoIDL[i].frameNr == detIDL[j].frameNr):
                filterIndex = j
                imageFound = True
                break
    ...
    sorted(allRects, key=annlib.functools.cmp_to_key(annlib.cmpDetAnnoRectsByScore))
    allRects.reverse()
...
def main():
    ...
    # Load files
    annoIDL = annlib.parse(annoFile)
    detIDL = []
    for dets in args[1:]:
        detIDL += annlib.parse(dets)

    if options.ignoreFile != None:
        ignoreIDL = annlib.parse(options.ignoreFile)
    else:
        ignoreIDL = copy.deepcopy(annoIDL)
        for anno in ignoreIDL:
            anno.rects = []
    ...
    if (not options.aspectRatio == None):
        annlib.forceAspectRatio(annoIDL, options.aspectRatio)
        annlib.forceAspectRatio(detIDL, options.aspectRatio)
        annlib.forceAspectRatio(ignoreIDL, options.aspectRatio)
    ...
    if outfilename is None:
        ...
        [base, ext] = annlib.idlBase(outputFile)
    ...
    if (options.analysisFile != None):
        ...
        falsePositives = annlib.AnnoList([])
        truePositives = annlib.AnnoList([])
        missingRecall = annlib.AnnoList([])
        ignoredTruePositives = annlib.AnnoList([])
        ...
        sortedFP = annlib.annoAnalyze(falsePositives)
        ...
        sortedFP = annlib.annoAnalyze(truePositives)
